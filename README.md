...
=======
>>>>>>> dfd866471c8b2573490ea08595aa406ca3e7fce3
>>>>>>> 0e84d8defffc0e098215dfb3441db6365b71cdce

Milestone 7 - README Appendix
==============================

Append this section to the project's existing README.md.
...
Fill in the PLACEHOLDER values by running each scenario at least
twice and recording the terminal output. Average the results.


Test scenario: heavy_traffic.txt
---------------------------------

Metric                              FCFS                    SJF
Total simulation wall-clock (s)     [PLACEHOLDER]           [PLACEHOLDER]
Average waiting time per node       [PLACEHOLDER] s         [PLACEHOLDER] s
Max queue depth observed            [PLACEHOLDER] travelers [PLACEHOLDER] travelers
Traveler with longest wait          PID [PLACEHOLDER]       PID [PLACEHOLDER]
Traveler with shortest wait         PID [PLACEHOLDER]       PID [PLACEHOLDER]
Order travelers entered Node N      [PLACEHOLDER]           [PLACEHOLDER]


Test scenario: scenario2.txt
------------------------------
...
Travelers are admitted to a node in strict arrival order.
Advantage: simple, starvation-free, fair.
Disadvantage: a traveler with a very long burst time can delay all
subsequent arrivals (convoy effect).
Expected behaviour in simulation: the order visible in the Waiting
Queue HUD panel matches the order the SYNC_WAITING IPC messages
arrived at the parent.

SJF (Shortest Job First):
Travelers are re-ordered by burst_time (ascending) before admission.
Advantage: minimises average waiting time across the queue.
Disadvantage: a long-burst traveler may be postponed indefinitely
if short-burst travelers keep arriving (starvation risk).
Expected behaviour in simulation: the queue panel re-orders entries
each time a new traveler joins; shorter-burst travelers bubble to
the front.

Observed difference (fill in after running):
[PLACEHOLDER: Describe in 2-3 sentences what you observed. Example:
Under SJF, traveler T3 (burst=1) entered Node 2 before T0 (burst=9)
despite T0 arriving first, reducing average node wait from X s to Y s.]


Demo Video Capture Guide (30-60 seconds)
==========================================

Record a single screen-capture session covering the following cues in
order. Narrate each step aloud so the reviewer can follow without
reading the terminal.

1.  Launch ./sim-schd fcfs heavy_traffic.txt
    Say: "Starting simulation with FCFS scheduler."

2.  Show top-right badge: "Scheduler Mode: FCFS"
    Say: "The badge confirms FCFS is active."

3.  Let 2-3 travelers block at the same node.
    Say: "Multiple travelers are queuing outside Node N."

4.  Point to the Waiting Queue HUD panel on the right side.
    Say: "Queue panel shows arrival order: T0, T2, T1."

5.  Watch a traveler enter the node (flash ring appears).
    Say: "T0 enters first - FCFS order preserved."

6.  Stop and relaunch with ./sim-schd sjf heavy_traffic.txt
    Say: "Now switching to SJF."

7.  Show top-right badge: "Scheduler Mode: SJF"
    Say: "Badge now shows SJF."

8.  Same node fills up - queue HUD shows reordered entries.
    Say: "T2 (burst=1) moved to position 0 despite arriving last."

9.  Shortest-burst traveler enters first (flash ring).
    Say: "T2 enters before T0 - SJF minimising wait time."

10. Terminal shows [M7] Node N queue (SJF): ...
    Say: "Console confirms SJF dispatch order."

11. Simulation ends with 2-second final frame.
    Say: "All travelers reached their destinations."


Tips for a clean recording:
Use simplescreenrecorder or ffmpeg -f x11grab on Linux.
Set window size to exactly 1280x720.
Trim silence from the start; total duration must be 30-60 seconds.
If the queue does not form naturally with heavy_traffic.txt, use the
built-in test scenario generated by generate_test_scenario("test_m7.txt").